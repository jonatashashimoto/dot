...
#define DURATION 100. // Increased duration to 20 seconds
// percentage of the duration for which the big glitch distortion is triggered
#define AMT_DISTORTION .1
...
void mainImage( out vec4 fragColor, in vec2 fragCoord )
{
    ...
    // smoothed interval for which the big glitch distortion gets triggered
    float glitchAmountDistortion = SS(DURATION * .001, DURATION * AMT_DISTORTION, mod(t, DURATION));
    ...
    // analog distortion (only active during glitchAmountDistortion)
    float y = uv.y * iResolution.y;
    float distortion = gnoise(vec3(0., y * .01, t * 500.)) * (glitchAmountDistortion * 4. + .1);
    distortion *= gnoise(vec3(0., y * .02, t * 250.)) * (glitchAmountDistortion * 2. + .025);

    ++displayNoise;
    distortion += smoothstep(.999, 1., sin((uv.y + t * 1.6) * 2.)) * 0.02 * glitchAmountDistortion;
    distortion -= smoothstep(.999, 1., sin((uv.y + t) * 2.)) * 0.02 * glitchAmountDistortion;
    st = uv + vec2(distortion, 0.);
    // chromatic aberration (also tied to the big distortion)
    ...
    // white noise + scanlines (constant and very subtle)
    displayNoise = 0.2 * clamp(displayNoise, 0., 1.);
    col += 0.01 * (hash33(vec3(fragCoord, mod(float(iFrame),
                    1000.))).r) * displayNoise;
    col -= 0.01 * (sin(4. * t + uv.y * iResolution.y * 1.75))
                    * displayNoise;
