			...
			routes = {
				{
					filter = {
						event = "msg_show",
						-- This matches the exact string you are seeing
						any = {
							{ find = "%[DBUI%]" },
							{ find = "Connected to db" },
						},
					},
					view = "notify", -- Changes it from a text line to a popup toast
					opts = { title = "Database", skip = false },
				},
			},
			-- Optional: If you want to see the SQL query progress in the mini view
			redirect = {
				view = "mini",
				filter = { event = "msg_show", find = "Executing" },
			},
			notify = {
				enabled = true,
				view = "notify",
